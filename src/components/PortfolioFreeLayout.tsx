...
function backgroundCss(bg: unknown): CSSProperties {
...
export function PortfolioFreeLayout({
  username,
  sections,
  pageBackground,
}: {
  ...
  const registerNode = useCallback((id: string, node: HTMLElement | null) => {
    ...
    const ro = new ResizeObserver((entries) => {
      ...
      if (!entry) return;
      const nextH = Math.ceil(entry.contentRect.height);
      setHeights((prev) =>
        prev[id] === nextH ? prev : { ...prev, [id]: nextH }
      );
    });
        ...
        <div className="relative" style={{ minHeight }}>
          {sorted.map((section) => {
            const layout = normalizeLayout(
              (section as { layout?: unknown }).layout
            );
            ...
            return (
              <div
                key={section.id}
                ref={(node) => registerNode(section.id, node)}
                className={`md:absolute ${wrapperClass()}`}
                style={wrapperStyle}
              >
                {section.type === "hero"
                  ? (() => {
                  ...
                  ? (() => {
                      const items = Array.isArray(section.content.items)
                        ? (section.content.items as unknown[]).map((x) =>
                            String(x)
                          )
                        : [];
                      return (
